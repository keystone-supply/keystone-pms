...
`check-blocked-packages.cjs` reads `package-lock.json` and exits with code `1` if any package in the `BLOCKED` list is present at a blocked version.
...
Or copy `check-blocked-packages.cjs` and `package-lock.json` to a temp directory, set a package entry to a blocked version in the copy, run `node scripts/security/check-blocked-packages.cjs` from that directory, and confirm exit code `1`.
