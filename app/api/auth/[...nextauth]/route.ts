...
async function refreshAccessToken(token: any) {
  try {
    ...
    console.log("🔁 refreshAccessToken called; refreshToken present:", !!token.refreshToken, "length:", token.refreshToken ? token.refreshToken.length : 0);
    const response = await fetch(`https://login.microsoftonline.com/${usedTenantId}/oauth2/v2.0/token`, {
      headers: {
    ...
    console.log('🔁 refresh response status', response.status, 'body:', tokens && typeof tokens === 'object' ? Object.keys(tokens) : tokens);
  ...
  callbacks: {
    async jwt({ token, account }) {
      ...
      if (account) {
        ...
        console.log("✅ Graph access token SAVED to JWT + expires:", new Date(token.accessTokenExpires).toISOString());
      }
      ...
      if (Date.now() > (token.accessTokenExpires ?? 0)) {
        ...
        if (refreshedTokens.access_token) {
          ...
          console.log("✅ Token refreshed successfully, new expiry:", new Date(token.accessTokenExpires).toISOString());
        } else {
          console.log("❌ Refresh failed, user will need to re-login", refreshedTokens);
        }
